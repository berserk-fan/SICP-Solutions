...
(define program
  '(begin
     (define (hello (a lazy) (b lazy-memo)) (if (= a 1) (begin a a) b))
     (hello (begin (display "a") 1) (display "hello\n"))))

(define program2
  '(begin
     (define (cons (x lazy) (y lazy)) (lambda (m) (m x y)))
     (define (car z)
       (define (px (x lazy) (y lazy)) x)
       (z px))
     (define
       (define (py (x lazy) (y lazy)) y)
       (z py))
     (define test (cons 1 (display "hello")))
     (display (car test))
     (display "\n")))
...
(define program3
  '(begin
     ...
     (define (map proc (items lazy))
       (if (null? items)
	   '()
	   (cons (proc (car items))
		 (map proc (cdr items)))))
     (define (scale-list (items lazy) factor)
